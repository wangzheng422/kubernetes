...
当 `ServerTLSBootstrap` 启用时，`certificate.Manager` 会自动处理服务证书的轮换。它会定期检查当前证书的过期时间。当过期时间临近时，管理器会启动新的 CSR 流程，从 API 服务器获取新的证书。根据 `client-go/util/certificate` 库中的实现，确切的轮换阈值是证书总有效期的 80% +/- 10%。这意味着当证书的剩余有效期在其总有效期的 10% 到 30% 之间时，会触发轮换。
...
// certificate.
```
对于一个签发了 30 天的证书，轮换将在证书过期前 3 到 9 天之间发生。

一旦收到并验证了新证书，`certificate.FileStore` 会更新磁盘上的文件，有效地用新证书替换旧证书。这确保了 Kubelet 的服务证书保持有效。`certificate_manager_server_rotation_seconds` 指标跟踪旧证书在轮换前的使用时长。
...
sequenceDiagram
    ...
    loop 证书轮换 (定期)
        ...
        KubeletCertificateManager->>KubeletCertificateManager: 计算剩余有效期
        alt 剩余有效期 <= 30% 且 >= 10% 的总有效期
